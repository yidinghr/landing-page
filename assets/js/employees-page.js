(function () {
  ...
  function getDisplayInterfaceTitle() {
    return state.interfaceMeta.title === dataApi.DEFAULT_INTERFACE_TITLE
      ? tr(dataApi.DEFAULT_INTERFACE_TITLE)
      : state.interfaceMeta.title;
  }

  function getDisplayInterfaceSubtitle() {
    const subtitle = state.interfaceMeta.subtitle || dataApi.DEFAULT_INTERFACE_SUBTITLE;
    return subtitle === dataApi.DEFAULT_INTERFACE_SUBTITLE
      ? tr(dataApi.DEFAULT_INTERFACE_SUBTITLE)
      : subtitle;
  }

  function getDisplayMainNote(departmentId) {
    const noteMap = state.interfaceMeta.mainNotes || {};
    const note = noteMap[departmentId];

    if (!note) {
      return tr(getDefaultMainNote(departmentId));
    }

    return note;
  }

  function getEditableTitleDraft() {
    return state.interfaceMeta.title === dataApi.DEFAULT_INTERFACE_TITLE
      ? tr(dataApi.DEFAULT_INTERFACE_TITLE)
      : state.interfaceMeta.title;
  }

  function getEditableSubtitleDraft() {
    const subtitle = state.interfaceMeta.subtitle || dataApi.DEFAULT_INTERFACE_SUBTITLE;
    return subtitle === dataApi.DEFAULT_INTERFACE_SUBTITLE
      ? tr(dataApi.DEFAULT_INTERFACE_SUBTITLE)
      : subtitle;
  ...
  function getEditableMainNoteDraft(departmentId) {
    const noteMap = state.interfaceMeta.mainNotes || {};
    return noteMap[departmentId] || tr(getDefaultMainNote(departmentId));
  }

  function renderLocaleControl() {
    if (!dom.pageTools || !i18n) {
      return;
    }
    ...
    dom.pageTools.innerHTML = [
      '<div class="yd-locale-control">',
      '<button type="button" class="yd-locale-button" data-locale-toggle="true" aria-label="' + escapeHtml(t("common.settings")) + '" aria-expanded="' + String(uiState.localeMenuOpen) + '">',
      '<span class="yd-locale-button__icon" aria-hidden="true">⚙</span>',
      "</button>",
      '<div class="yd-locale-popover"' + (uiState.localeMenuOpen ? "" : " hidden") + '>',
      '<p class="yd-locale-popover__title">' + escapeHtml(t("common.language")) + "</p>",
      options,
      "</div>",
      "</div>"
    ].join("");
  ...
  function buildShell() {
    document.title = t("employees.pageTitle");
    root.innerHTML = [
      '<div class="employees-app">',
      '<div id="employeesPageTools" class="yd-page-tools"></div>',
    ...
    dom.pageTools = document.getElementById("employeesPageTools");
  }
  ...
  function formatTabConditionSummary(condition) {
    ...
    return [tr(field.label), operator ? tr(operator.label) : "", condition.value].filter(Boolean).join(" ");
  }
  ...
  function renderSidebar() {
    const selectedDepartmentId = state.selectedDepartmentId;
    const subtitleText = getDisplayInterfaceSubtitle();

    dom.sidebarMount.innerHTML = [
      '<div class="employees-sidebar__header">',
      '<div class="employees-sidebar__icon-wrap">',
      '<button type="button" class="employees-sidebar__icon-button" data-action="choose-interface-icon" data-tooltip="' + escapeHtml(tr("點擊更換介面圖片")) + '">',
      '<img class="employees-sidebar__icon-preview" src="' + escapeHtml(state.interfaceMeta.iconSrc) + '" alt="' + escapeHtml(tr("介面圖示")) + '">',
      "</button>",
      state.interfaceMeta.customIcon
        ? '<div class="employees-sidebar__icon-tools"><button type="button" class="employees-icon-button employees-icon-button--ghost employees-icon-button--danger" data-action="reset-interface-icon" aria-label="' + escapeHtml(tr("還原介面圖片")) + '">' + getIconSvg("trash") + "</button></div>"
        : "",
      "</div>",
      ...
      uiState.editingTitle
        ...
        : '<h1 class="employees-sidebar__title">' + escapeHtml(getDisplayInterfaceTitle()) + "</h1>",
      '<div class="employees-sidebar__title-actions">',
      '<button type="button" class="employees-icon-button employees-icon-button--ghost" data-action="toggle-title-edit" aria-label="' + escapeHtml(tr("編輯標題")) + '">' + getIconSvg("edit") + "</button>",
      "</div>",
      "</div>",
      ...
      '<div class="employees-sidebar__title-actions">',
      '<button type="button" class="employees-icon-button employees-icon-button--ghost" data-action="toggle-subtitle-edit" aria-label="' + escapeHtml(tr("編輯說明")) + '">' + getIconSvg("edit") + "</button>",
      "</div>",
      "</div>",
      "</div>",
      "</div>",
      '<div class="employees-sidebar__body">',
      '<h2 class="employees-sidebar__section-title">' + escapeHtml(tr("部門")) + "</h2>",
      ...
      state.departments.map(function (department) {
        ...
        return [
          ...
          '<button type="button" class="employees-icon-button employees-icon-button--ghost employees-department__menu-button" data-action="toggle-department-menu" data-department-id="' + escapeHtml(department.id) + '" aria-label="' + escapeHtml(tr("部門選單")) + '">' + getIconSvg("more") + "</button>",
          isMenuOpen
            ? '<div class="employees-department__menu-panel"><button type="button" data-action="start-edit-department" data-department-id="' + escapeHtml(department.id) + '">' + escapeHtml(tr("編輯")) + '</button><button type="button" data-action="delete-department" data-department-id="' + escapeHtml(department.id) + '">' + escapeHtml(tr("刪除")) + "</button></div>"
            : "",
          "</div>",
          "</div>"
        ].join("");
      ...
      uiState.addingDepartment
        ? '<input id="employeesDepartmentInput" class="employees-department-add__input" type="text" value="' + escapeHtml(uiState.departmentDraft) + '" placeholder="' + escapeHtml(tr("請輸入部門名稱")) + '">'
        : '<button type="button" class="employees-department-add__button" data-action="start-add-department"><span>' + getIconSvg("plus") + "</span><strong>" + escapeHtml(tr("新增部門")) + "</strong></button>",
      "</div>",
      "</div>",
      "</div>"
    ].join("");
  ...
  function renderMainHeader() {
    ...
    const mainNote = getDisplayMainNote(selectedDepartment.id);
    ...
    dom.mainHeaderMount.innerHTML = [
      ...
      '<button type="button" class="employees-icon-button employees-icon-button--ghost" data-action="toggle-main-note-edit" aria-label="' + escapeHtml(tr("編輯主區說明")) + '">' + getIconSvg("edit") + "</button>",
      "</div>",
      "</div>",
      "</div>",
  ...
  function renderFilterPopover() {
    return [
      ...
      '<div class="employees-popover__section-title">' + escapeHtml(tr("篩選")) + "</div>",
      '<div class="employees-popover__field"><label>' + escapeHtml(tr("職位")) + '</label><select data-setting="filter-position"><option value="全部">' + escapeHtml(tr("全部")) + "</option>" + dataApi.POSITION_OPTIONS.map(function (option) {
        return '<option value="' + escapeHtml(option) + '"' + (state.filters.position === option ? " selected" : "") + ">" + escapeHtml(option) + "</option>";
      }).join("") + "</select></div>",
      '<div class="employees-popover__field"><label>' + escapeHtml(tr("狀態")) + '</label><select data-setting="filter-status">' +
      ['全部'].concat(dataApi.STATUS_OPTIONS).map(function (option) {
        return '<option value="' + escapeHtml(option) + '"' + (state.filters.status === option ? " selected" : "") + ">" + escapeHtml(option === "全部" ? tr(option) : option) + "</option>";
      }).join("") +
      "</select></div>",
      '<button type="button" class="employees-inline-action" data-action="clear-filters">' + escapeHtml(tr("清除篩選")) + "</button>",
      "</div>"
    ].join("");
  }
  ...
  function renderDisplayPopover() {
    ...
    return [
      '<div class="employees-popover" data-modal-body="true">',
      '<div class="employees-popover__section-title">' + escapeHtml(tr("卡片顯示")) + "</div>",
      '<div class="employees-popover__field"><label>' + escapeHtml(tr("主標題")) + '</label><select data-setting="card-title">' + dataApi.CARD_FIELD_OPTIONS.map(function (field) {
        return '<option value="' + escapeHtml(field.id) + '"' + (state.cardDisplay.titleField === field.id ? " selected" : "") + ">" + escapeHtml(tr(field.label)) + "</option>";
      }).join("") + "</select></div>",
      state.cardDisplay.extraFieldIds.map(function (fieldId, index) {
        return '<div class="employees-popover__field"><label>' + escapeHtml(tr("欄位 " + String(index + 1))) + '</label><select data-setting="card-extra-' + String(index) + '">' + dataApi.CARD_FIELD_OPTIONS.map(function (field) {
          return '<option value="' + escapeHtml(field.id) + '"' + (fieldId === field.id ? " selected" : "") + ">" + escapeHtml(tr(field.label)) + "</option>";
        }).join("") + "</select></div>";
      }).join(""),
      '<div class="employees-popover__section-title">' + escapeHtml(tr("排序")) + "</div>",
      '<div class="employees-popover__field"><label>' + escapeHtml(tr("排序方式")) + '</label><select data-setting="sort-mode">' + dataApi.SORT_OPTIONS.filter(function (option) {
        return !isRetiredView() || option.id === "retiredSoonest";
      }).map(function (option) {
        return '<option value="' + escapeHtml(option.id) + '"' + (selectedSort === option.id ? " selected" : "") + ">" + escapeHtml(tr(option.label)) + "</option>";
      }).join("") + "</select></div>",
      "</div>"
    ].join("");
  }
  ...
  function renderTabValueControl(condition, conditionIndex) {
    ...
    if (draftField.type === "select") {
      return '<div class="employees-toolbar__composer-field"><label>' + escapeHtml(tr("內容")) + '</label><select ' + inputAttrs + '>' + getFieldOptionList(draftField.id).map(function (option) {
        return '<option value="' + escapeHtml(option) + '"' + (condition.value === option ? " selected" : "") + ">" + escapeHtml(option) + "</option>";
      }).join("") + "</select></div>";
    }

    if (draftField.type === "date") {
      return '<div class="employees-toolbar__composer-field"><label>' + escapeHtml(tr("日期")) + '</label><input type="text" ' + inputAttrs + ' value="' + escapeHtml(condition.value) + '" placeholder="2026-03-31"></div>';
    }

    if (draftField.type === "number") {
      return '<div class="employees-toolbar__composer-field"><label>' + escapeHtml(tr("數值")) + '</label><input type="number" ' + inputAttrs + ' value="' + escapeHtml(condition.value) + '" placeholder="0"></div>';
    }

    return '<div class="employees-toolbar__composer-field"><label>' + escapeHtml(tr("內容")) + '</label><input type="text" ' + inputAttrs + ' value="' + escapeHtml(condition.value) + '" placeholder="' + escapeHtml(tr("請輸入條件")) + '"></div>';
  }

  function renderTabConditionComposer(condition, conditionIndex, totalConditions) {
    return [
      '<div class="employees-toolbar__condition-row">',
      '<div class="employees-toolbar__composer-field"><label>' + escapeHtml(tr("欄位")) + '</label><select data-tab-condition-index="' + String(conditionIndex) + '" data-tab-condition-key="fieldId">' + TAB_FILTER_FIELDS.map(function (field) {
        return '<option value="' + escapeHtml(field.id) + '"' + (condition.fieldId === field.id ? " selected" : "") + ">" + escapeHtml(tr(field.label)) + "</option>";
      }).join("") + "</select></div>",
      '<div class="employees-toolbar__composer-field"><label>' + escapeHtml(tr("條件")) + '</label><select data-tab-condition-index="' + String(conditionIndex) + '" data-tab-condition-key="operator">' + getTabOperators(condition.fieldId).map(function (operator) {
        return '<option value="' + escapeHtml(operator.id) + '"' + (condition.operator === operator.id ? " selected" : "") + ">" + escapeHtml(tr(operator.label)) + "</option>";
      }).join("") + "</select></div>",
      renderTabValueControl(condition, conditionIndex),
      '<div class="employees-toolbar__condition-actions">' +
        (totalConditions > 1
          ? '<button type="button" class="employees-icon-button employees-icon-button--ghost employees-icon-button--danger" data-action="remove-tab-condition" data-tab-condition-index="' + String(conditionIndex) + '" aria-label="' + escapeHtml(tr("移除條件")) + '">✕</button>'
          : "") +
      "</div>",
      "</div>"
    ].join("");
  ...
  function renderToolbar() {
    ...
    if (isRetiredView()) {
      dom.toolbarMount.innerHTML = [
        ...
        uiState.showSearchInput ? '<input class="employees-search-box" data-path="searchQuery" value="' + escapeHtml(state.searchQuery) + '" placeholder="' + escapeHtml(tr("搜尋員工")) + '">' : "",
        '<button type="button" class="employees-tool-icon" data-action="toggle-search" data-tooltip="' + escapeHtml(tr("搜尋")) + '">' + getIconSvg("search") + "</button>",
        '<div class="employees-tool-group" data-tool-group="filter">',
        '<button type="button" class="employees-tool-icon" data-action="toggle-filter-menu" data-tooltip="' + escapeHtml(tr("篩選")) + '">' + getIconSvg("filter") + "</button>",
    ...
    dom.toolbarMount.innerHTML = [
      ...
      visibleTabsData.hiddenTabs.length
        ? '<div class="employees-tool-group" data-tool-group="more-tabs"><button type="button" class="employees-tab" data-action="toggle-more-tabs">' + escapeHtml(tr("更多")) + "</button>" +
          ...
          "</div>"
        : "",
      '<button type="button" class="employees-tab" data-action="start-create-tab" title="' + escapeHtml(tr("新增分頁")) + '">' + getIconSvg("plus") + "</button>",
      activeTab && !uiState.creatingTab
        ? '<div class="employees-tabs__actions"><button type="button" class="employees-icon-button employees-icon-button--ghost" data-action="start-edit-tab" aria-label="' + escapeHtml(tr("編輯分頁")) + '">' + getIconSvg("edit") + '</button><button type="button" class="employees-icon-button employees-icon-button--ghost employees-icon-button--danger" data-action="delete-active-tab" aria-label="' + escapeHtml(tr("刪除分頁")) + '">✕</button></div>'
        : "",
      "</div>",
      uiState.creatingTab
        ? '<div class="employees-toolbar__composer">' +
          '<div class="employees-toolbar__composer-field employees-toolbar__composer-field--wide"><label>' + escapeHtml(tr("分頁名稱")) + '</label><input type="text" id="employeesTabNameInput" value="' + escapeHtml(uiState.tabDraft.name) + '" placeholder="' + escapeHtml(tr("可留空，由條件自動命名")) + '"></div>' +
          '<div class="employees-toolbar__composer-note">' + escapeHtml(tr("同一分頁內的條件必須全部成立，才會顯示在這個分頁。")) + "</div>" +
          ...
          '<div class="employees-toolbar__composer-footer"><button type="button" class="employees-secondary-button" data-action="add-tab-condition">' + getIconSvg("plus") + "<span>" + escapeHtml(tr("新增條件")) + '</span></button><button type="button" class="employees-secondary-button" data-action="cancel-tab-composer">' + escapeHtml(tr("取消")) + '</button><button type="button" class="employees-primary-button employees-toolbar__composer-confirm" data-action="confirm-create-tab">' + escapeHtml(tr(uiState.editingTabId ? "儲存分頁" : "建立分頁")) + "</button></div>" +
          "</div>"
        : "",
      "</div>",
      '<div class="employees-toolbar__right">',
      ...
      '<button type="button" class="employees-tool-icon" data-action="toggle-display-menu" data-tooltip="' + escapeHtml(tr("顯示設定")) + '">' + getIconSvg("more") + "</button>",
  ...
  function renderCards() {
    ...
    if (!employees.length) {
      dom.cardsMount.innerHTML = '<div class="employees-cards-panel"><div class="employees-empty">' + escapeHtml(tr("目前沒有符合條件的員工資料。")) + "</div></div>";
      return;
    }
    ...
    dom.cardsMount.innerHTML = '<div class="employees-cards-panel"><div class="employees-cards">' + employees.map(function (employee) {
      const lines = state.cardDisplay.extraFieldIds.map(function (fieldId) {
        ...
        return [
          ...
          '<div class="employees-card__label">' + escapeHtml(fieldOption ? tr(fieldOption.label) : fieldId) + "</div>",
      ...
      return [
        ...
        '<img class="employees-card__avatar" data-image-kind="employee-avatar" src="' + escapeHtml(employee.avatarSrc || dataApi.DEFAULT_IMAGE_SRC) + '" alt="' + escapeHtml(employee.basic.engName || employee.basic.vieName || tr("員工頭像")) + '">',
    ...
    dom.detailMount.innerHTML = [
      ...
      '<div class="employees-detail__actions-right">' +
        (selectedEmployee && uiState.detailMode !== "add"
          ? '<button type="button" class="employees-secondary-button employees-primary-button--danger" data-action="request-delete-employee">' + escapeHtml(tr("刪除")) + "</button>"
          : "") +
        (!inRetired ? '<button type="button" class="employees-primary-button" data-action="open-add-panel">' + getIconSvg("plus") + "<span>" + escapeHtml(tr("新增")) + "</span></button>" : "") +
      "</div>",
      "</div>",
      '<div class="employees-detail__content">',
      '<div class="employees-avatar-box">',
      '<div class="employees-avatar-box__preview-wrap">',
      '<button type="button" class="employees-avatar-box__preview" data-action="preview-avatar" data-tooltip="' + escapeHtml(tr(isEditable ? "點擊查看或更換頭像" : "點擊查看頭像")) + '">',
      '<img data-image-kind="employee-avatar" src="' + escapeHtml(draft.avatarSrc || dataApi.DEFAULT_IMAGE_SRC) + '" alt="' + escapeHtml(tr("員工頭像")) + '">',
      "</button>",
      "</div>",
      '<div class="employees-avatar-box__meta">' + escapeHtml(tr(isEditable ? "點擊頭像可預覽或更換" : "點擊頭像可放大預覽")) + "</div>",
      "</div>",
      ...
      !inRetired ? [
        ...
        '<button type="button" class="employees-primary-button" data-action="save-employee"' + (uiState.detailMode === "view" ? " disabled" : "") + ">" + escapeHtml(tr("儲存")) + "</button>",
        '<button type="button" class="employees-secondary-button" data-action="start-edit-employee"' + (uiState.detailMode !== "view" ? " disabled" : "") + ">" + escapeHtml(tr("編輯")) + "</button>",
        "</div>"
      ].join("") : ""
    ].join("");
  ...
  function renderModal() {
    ...
    if (uiState.openModal === "confirm-delete") {
      dom.modalMount.innerHTML = [
        ...
        '<h2 class="employees-modal__title">' + escapeHtml(tr("確認刪除")) + "</h2>",
        '<div class="employees-modal__text">' + escapeHtml(tr("確定要刪除此員工資料嗎？")) + "</div>",
        '<div class="employees-modal__actions">',
        '<button type="button" class="employees-secondary-button" data-action="close-modal">' + escapeHtml(tr("取消")) + "</button>",
        '<button type="button" class="employees-primary-button employees-primary-button--danger" data-action="open-password-modal">' + escapeHtml(tr("確認")) + "</button>",
        "</div></div></div>"
      ].join("");
      return;
    }
    ...
    if (uiState.openModal === "password-delete") {
      dom.modalMount.innerHTML = [
        '<div class="employees-modal"><div class="employees-modal__card" data-modal-body="true">',
        '<h2 class="employees-modal__title">' + escapeHtml(tr("請輸入刪除密碼")) + "</h2>",
        '<div class="employees-modal__text">' + escapeHtml(tr("密碼固定為：091100")) + "</div>",
        ...
        '<div class="employees-modal__actions">',
        '<button type="button" class="employees-secondary-button" data-action="close-modal">' + escapeHtml(tr("取消")) + "</button>",
        '<button type="button" class="employees-primary-button employees-primary-button--danger" data-action="confirm-delete-employee">' + escapeHtml(tr("刪除")) + "</button>",
        "</div></div></div>"
      ].join("");
      return;
    }
    ...
    if (uiState.openModal === "password-delete-attachment") {
      dom.modalMount.innerHTML = [
        '<div class="employees-modal"><div class="employees-modal__card" data-modal-body="true">',
        '<h2 class="employees-modal__title">' + escapeHtml(tr("請輸入檔案刪除密碼")) + "</h2>",
        '<div class="employees-modal__text">' + escapeHtml(tr("密碼固定為：09110")) + "</div>",
        '<input id="employeesPasswordInput" class="employees-modal__input" type="password" value="' + escapeHtml(uiState.passwordDraft) + '">',
        uiState.passwordError ? '<div class="employees-modal__text" style="color:#ff8a80;">' + escapeHtml(uiState.passwordError) + "</div>" : "",
        '<div class="employees-modal__actions">',
        '<button type="button" class="employees-secondary-button" data-action="close-modal">' + escapeHtml(tr("取消")) + "</button>",
        '<button type="button" class="employees-primary-button employees-primary-button--danger" data-action="confirm-delete-attachment">' + escapeHtml(tr("刪除")) + "</button>",
        "</div></div></div>"
      ].join("");
      return;
    }
    ...
    if (uiState.openModal === "confirm-close") {
      dom.modalMount.innerHTML = [
        '<div class="employees-modal"><div class="employees-modal__card" data-modal-body="true">',
        '<h2 class="employees-modal__title">' + escapeHtml(tr("確認關閉")) + "</h2>",
        '<div class="employees-modal__text">' + escapeHtml(tr("各項資料可能不會儲存，是否確認繼續？")) + "</div>",
        '<div class="employees-modal__actions">',
        '<button type="button" class="employees-secondary-button" data-action="close-modal">' + escapeHtml(tr("取消")) + "</button>",
        '<button type="button" class="employees-primary-button" data-action="confirm-close-panel">' + escapeHtml(tr("確認")) + "</button>",
        "</div></div></div>"
      ].join("");
      return;
    }
    ...
    if (uiState.openModal === "notice") {
      dom.modalMount.innerHTML = [
        '<div class="employees-modal"><div class="employees-modal__card" data-modal-body="true">',
        '<h2 class="employees-modal__title">' + escapeHtml(tr("提示")) + "</h2>",
        '<div class="employees-modal__text">' + escapeHtml(uiState.noticeText) + "</div>",
        '<div class="employees-modal__actions"><button type="button" class="employees-primary-button" data-action="close-modal">' + escapeHtml(tr("確認")) + "</button></div>",
        "</div></div>"
      ].join("");
      return;
    }
    ...
    if (uiState.openModal === "preview-avatar") {
      dom.modalMount.innerHTML = [
        ...
        '<button type="button" class="employees-image-preview__backdrop" data-action="close-modal" aria-label="' + escapeHtml(tr("關閉預覽")) + '"></button>',
        '<div class="employees-image-preview" data-modal-body="true">',
        '<div class="employees-image-preview__frame-wrap">',
        '<button type="button" class="employees-icon-button employees-icon-button--ghost employees-image-preview__close" data-action="close-modal" aria-label="' + escapeHtml(tr("關閉預覽")) + '">✕</button>',
        '<div class="employees-image-preview__frame">',
        '<img data-image-kind="employee-avatar" src="' + escapeHtml(uiState.previewImageSrc) + '" alt="' + escapeHtml(tr("頭像預覽")) + '">',
        "</div></div>",
        '<div class="employees-image-preview__actions">' +
          (uiState.detailMode === "add" || uiState.detailMode === "edit"
            ? '<button type="button" class="employees-secondary-button" data-action="choose-avatar">' + escapeHtml(tr("更換")) + "</button>" +
              (uiState.draftEmployee && uiState.draftEmployee.avatarChanged ? '<button type="button" class="employees-secondary-button employees-primary-button--danger" data-action="reset-avatar">' + escapeHtml(tr("還原")) + "</button>" : "")
            : "") +
        "</div>",
        "</div></div>"
      ].join("");
  ...
  function toggleTitleEdit() {
    ...
    uiState.titleDraft = uiState.editingTitle ? getEditableTitleDraft() : "";
    renderSidebar();
    focusPendingInputs();
  }
  ...
  function toggleSubtitleEdit() {
    ...
    uiState.subtitleDraft = uiState.editingSubtitle ? getEditableSubtitleDraft() : "";
    renderSidebar();
    focusPendingInputs();
  }
  ...
  function toggleMainNoteEdit() {
    ...
    uiState.mainNoteDraft = uiState.editingMainNote ? getEditableMainNoteDraft(state.selectedDepartmentId) : "";
    renderMainHeader();
    focusPendingInputs();
  }
  ...
  function deleteDepartment(departmentId) {
    ...
    if (hasEmployees) {
      openNotice(tr("此部門仍有員工資料，請先處理員工後再刪除。"));
      return;
    }
  ...
  function handleRootClick(event) {
    ...
    if (!actionButton) {
      ...
      if (uiState.localeMenuOpen && !event.target.closest("#employeesPageTools")) {
        uiState.localeMenuOpen = false;
        renderLocaleControl();
      }
